...
import { encodeSurpriseForLink } from '@/utils/shareCode'
...
export default function CreateSurprise() {
  ...
  const Experience = useMemo(() => getExperience(occasionId), [occasionId])

  if (!occasion) {
    return (
      <div className="mx-auto max-w-md px-6 py-24 text-center">
        <p className="font-display text-2xl">We couldn't find that occasion.</p>
  ...
  function handleCreate(e) {
    ...
    const surprise = { ...form, occasionId }

    // Save locally too, so this device's "My surprises" list still works.
    try {
      saveSurprise(slug, surprise)
    } catch {
      // Storage full or unavailable — the link below still works since the
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-5 pb-24 sm:px-8">
        <div className="mb-8">
          <Link to="/" className="text-sm text-cream/50 hover:text-cream/80">
            ← Choose a different occasion
          </Link>
          <div className="mt-3 flex items-center gap-3">
            <span className="text-3xl">{occasion.emoji}</span>
            <h1 className="font-display text-3xl sm:text-4xl">{occasion.label} surprise</h1>
          </div>
        </div>
          ...
          {['edit', 'preview'].map((tab) => (
            <button
              ...
              className={`flex-1 rounded-full py-2 text-sm font-medium capitalize transition-colors ${
                mobileTab === tab ? 'bg-rose text-ink' : 'text-cream/60'
              }`}
            >
              {tab}
            </button>
        ...
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_1fr]">
          <motion.form
            ...
            className={`${mobileTab === 'edit' ? 'block' : 'hidden'} space-y-6 lg:block`}
          >
            ...
            <Field label={occasionId === 'anniversary' ? 'Date it all began' : 'Date (optional)'}>
              <input
                type="date"
                value={form.date}
            ...
            <Field label="Theme">
              <ThemeSelector value={form.theme} onChange={(t) => update('theme', t)} />
            </Field>

            <Field label="Music">
              <MusicUploader
                fileName={form.musicName}
                onChange={(src, name) => setForm((f) => ({ ...f, musicSrc: src, musicName: name }))}
              />
            </Field>
          ...
          <div className={`${mobileTab === 'preview' ? 'block' : 'hidden'} lg:block`}>
            <div className="sticky top-6">
              <p className="mb-3 text-xs uppercase tracking-widest text-cream/40">Live preview</p>
        ...
        <div className="mt-16">
          <p className="mb-3 text-xs uppercase tracking-widest text-cream/40">Other occasions</p>
          <div className="flex flex-wrap gap-2">
            {OCCASIONS.filter((o) => o.id !== occasionId).map((o) => (
              <Link
                key={o.id}
                to={`/create/${o.id}`}
